...
void Camera::setCallback(void* callbackFunction) {

	int success = false;

	// Select the window for video rendering.
	success = cvcamSetProperty(cameraIndex, CVCAM_PROP_CALLBACK, &callbackFunction);
	
	if(DEBUG && success) debug("(!) Unable to link callback");
